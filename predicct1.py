...
def evaluate_normal(model, x_data, y_data):
    ...
    for i, (x, y) in tqdm(enumerate(zip(x_data, y_data)), total=len(x_data)):
        ...
        y_pred1 = parse(model.predict(x)[0])  # segmentation 结果
        y_pred2 = parse(model.predict(x)[1])  # explanation 结果
...
if __name__ == "__main__":
    ...
    test_dataset = create_dataset(test_x, test_y, batch=batch_size)
